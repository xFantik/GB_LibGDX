...
//    }
    ...
    public static Character getDino(Main main){
        if (dino== null)    {
            AnimTexture anim;
            anim = new AnimTexture("dino.png", 1 / 10f, 8,7);
            anim.initializeAction(Movable.Actions.IDLE, 0, 15, Animation.PlayMode.LOOP, true);
            anim.initializeAction(Movable.Actions.RUN, 17, 25, Animation.PlayMode.LOOP, true);
            anim.initializeAction(Movable.Actions.DEAD, 25, 36, Animation.PlayMode.NORMAL, true);
            anim.initializeAction(Movable.Actions.JUMP, 41, 49, Animation.PlayMode.LOOP, true);
    ...
    public static Character getGreenDino(Main main){
        if (greenDino ==null){
            ...
            anim.initializeAction(Movable.Actions.IDLE, "Idle", Animation.PlayMode.LOOP, false,false);
            anim.initializeAction(Movable.Actions.RUN, "Run", Animation.PlayMode.LOOP, false,false);
            anim.initializeAction(Movable.Actions.DEAD, "Dead", Animation.PlayMode.NORMAL, false,false);
            anim.initializeAction(Movable.Actions.JUMP,"Jump", Animation.PlayMode.LOOP, false,false);
